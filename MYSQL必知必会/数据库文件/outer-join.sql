...
WHERE c.cust_id = o.cust_id AND o.order_num = oi.order_num AND oi.prod_id = 'TNT2';

#使用子查询方式筛选数据
SELECT prod_id,prod_name 
FROM products WHERE vend_id IN(SELECT vend_id FROM products WHERE prod_id = 'DTNTR');
...
SELECT c.cust_name AS 客户名,c.cust_id AS 客户ID,COUNT(o.order_num) AS 客户订单 
FROM customers AS c LEFT OUTER JOIN orders AS o ON c.cust_id = o.cust_id 
GROUP BY 客户名 ORDER BY 客户订单 DESC;
