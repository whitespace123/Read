...
SELECT * FROM gjp_zhangwu WHERE zwname REGEXP '[股票工资礼金]收入' 
ORDER BY zwmoney DESC LIMIT 5;
#与上同理
SELECT * FROM gjp_zhangwu WHERE zwname REGEXP '股票|工资|礼金 收入'
ORDER BY zwmoney DESC LIMIT 5;

#使用正则表达式来筛选匹配不含有工资收入和礼金收入的数据，用降序显示前5条记录
SELECT * FROM gjp_zhangwu WHERE zwname REGEXP '[^工资礼金]收入'
ORDER BY zwmoney DESC LIMIT 5;
